...
## Recent Updates
- **Enhanced Client Recommendations Page (Oct 12, 2025)**: Added billing transparency to build client trust. Pricing/hours now hidden until admin approval (only shown when status === "Awaiting Approval"). Clear explanatory text added below cost/hours: "Upon approval, this will generate an invoice for $X" for cost-based initiatives, or "Upon approval, X hours will be deducted from your monthly retainer" for hour-based initiatives. Billing info appears in highlighted box (bg-muted/50 border) with improved visual hierarchy.
