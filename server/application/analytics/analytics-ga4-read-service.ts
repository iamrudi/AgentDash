...
export interface AnalyticsGa4Result<T> {
  ok: boolean;
  status: number;
  data?: T;
  error?: string;
...
export class AnalyticsGa4ReadService {
  constructor(
    private readonly storage: IStorage,
    private readonly deps: AnalyticsGa4Deps
  ) {}
  ...
  private async loadActiveIntegration(clientId: string): Promise<AnalyticsGa4Result<Integration>> {
    let integration = await this.storage.getIntegrationByClientId(clientId, "GA4");
    if (!integration || !integration.ga4PropertyId) {
      return { ok: false, status: 404, error: "GA4 integration not configured" };
    }
  ...
  async getConversions(params: {
    ...
    startDate?: string;
    endDate?: string;
  }): Promise<AnalyticsGa4Result<unknown>> {
    ...
    if (!integration.ga4LeadEventName) {
      return {
        ok: false,
        status: 400,
        error: "Lead event name not configured. Please configure it in the integrations page.",
      };
    }

    const { start, end } = resolveDateRange(params.startDate, params.endDate);
    const data = await this.deps.fetchGA4KeyEvents(
  ...
  async getChannels(params: {
    ...
    const data = await this.deps.fetchGA4AcquisitionChannels(
      integration.accessToken!,
      integration.ga4PropertyId!,
      start,
      end,
  ...
  async getAnalytics(params: {
    ...
    const data = await this.deps.fetchGA4Data(
      integration.accessToken!,
      integration.ga4PropertyId!,
      start,
      end,
